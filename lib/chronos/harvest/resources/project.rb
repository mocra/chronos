class Harvest
  module Resources
    ...
    class Project < Harvest::Resource
      ...
      class << self
        def find_id_by_name(name)
          obj = all.detect { |p| p.name == name }
          raise ActiveResource::ResourceNotFound, "A project with the name #{name} could not be found." if obj.nil?
          obj.id
        end
      end
